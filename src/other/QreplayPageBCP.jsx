...
const QReplayPage = () => {
    ...
    useEffect(() => {
        ...
        const fetchReports = async () => {
            try {
                const [tripsResponse, summaryResponse, eventsResponse, positionsResponse] =
                    await Promise.all([
                ...
                const [tripData, summaryData, events, reportPositions] = await Promise.all([
                    tripsResponse.json(),
                ...
                const overspeedEvents = events.filter((event) => event.type === 'deviceOverspeed');
                ...
                setTopSpeed(
                    overspeedEvents.length
                        ? Math.max(...overspeedEvents.map((event) => event.attributes?.speed ?? 0))
                        : 0,
                );
        ...
        return () => controller.abort();
    }, [eventsReportQuery, reportQuery, replay]);
                                        ...
                                        <Fragment key={`${trip.startTime}-${trip.endTime}-${index}`}>
                                            <ListItemButton
                                                sx={{
                                                    ...
                                                    borderLeft: `4px solid ${TRIP_COLOR}`,
                                                    paddingLeft: 1.5,
                                                    '&.Mui-selected': {
                                            ...
                                            {hasStop && (
                                                <ListItemButton
                                                    ...
                                                    sx={{
                                                        borderBottom: '1px solid #d4d4d4ff',
                                                        borderLeft: `4px solid ${STOP_COLOR}`,
